...
Shader "Water/SeaWave" {
	...
	SubShader {
		...
		void surf (Input IN, inout SurfaceOutput o) {
			...
			uv.y = 1 - uv.y;
			#endif
			fixed4 water = tex2D(_WaterTex, IN.uv_WaterTex);
			...
			float4 offsetColor = tex2D(_BumpTex, IN.uv_WaterTex); 
			...
			//替换
			//half4 bott = tex2D(_MainTex, uv);
			half4 bott = tex2D(_GrabTexture, uv);

			//_GTex替换
			//fixed4 waterColor = tex2D(_MainTex, float2(min(_Range.y, deltaDepth)/_Range.y,1));
